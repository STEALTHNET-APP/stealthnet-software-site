...
<p>The installer issues no certificates: the install is the same for every node, with no
branches. Reality — the default choice — needs none at all, since it hides behind somebody
else's site.</p>
<p>A certificate is only needed by what runs over TLS: Trojan, Hysteria2 and the like. There
it is tied to the domain of one particular node, so it is issued on the spot.</p>
<p>Point a domain's A record at the node, free up port 80, and issue it:</p>
<pre><code>apt-get install -y certbot
...
cp /etc/letsencrypt/live/de1.your-domain/privkey.pem   /etc/sn-node/tls/de1.your-domain.key</code></pre>
<p>Then point the profile at those paths, in
<code>streamSettings.tlsSettings.certificates</code>.</p>

<div class="note warn"><b>The path is to <u>this node's</u> certificate, not the panel's
domain.</b> The Hysteria2 preset ships with a placeholder; leave the panel's domain in it and
the engine on the node answers <code>no such file or directory</code> and refuses to start —
taking the profile's other inbounds with it.</div>

<div class="note"><b>One profile across nodes.</b> The path in the config is a single value, so
a TLS inbound cannot live in a profile shared by nodes with different domains. Give those nodes
a profile of their own.</div>

<div class="note"><b>After a renewal</b> the engine has to be restarted: it reads the
certificate only at start-up and would otherwise keep serving an expired one. There is a
"Restart Xray" action in the node menu; in a certbot job it is
<code>--deploy-hook "systemctl restart sn-node"</code>.</div>

<h2 id="update">Updating</h2>
<p>The engine version is set in the panel: the <b>“Xray version”</b> button on the nodes page.
The agent compares its own on every poll, downloads the release, makes sure the binary runs
and reports the expected version, and only then replaces the working one. Nodes left behind
are listed there too.</p>
<p>The agent itself is updated with <b>“Apply to every node”</b> in the same window. It
replaces itself and exits — the service brings it back up.</p>

<div class="note warn"><b>A node with an old agent does not understand the panel's commands</b>
and stays on its previous engine — in the list this shows up as mismatched versions. Such a
node has to be updated once, on the server itself:</div>
<pre><code>curl -fsSL https://panel.your-domain/update-node.sh | PANEL_URL=https://panel.your-domain bash</code></pre>
<p>The node secret is not needed: it is already in the systemd unit. The script leaves the
engine alone — the panel owns its version and the agent will bring the node to it. To pin a
specific version, name it: <code>XRAY_VERSION=v26.7.11</code>.</p>
